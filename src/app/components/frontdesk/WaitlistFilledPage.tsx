...
function DonutLabel({ viewBox, total }: { viewBox?: { cx: number; cy: number }; total: number }) {
  ...
  return (
    <>
      <text x={cx} y={cy - 6} textAnchor="middle" dominantBaseline="middle" className="fill-foreground text-[18px] font-medium" fontSize={18} fontWeight={500}>
        {total}
      </text>
      <text x={cx} y={cy + 14} textAnchor="middle" dominantBaseline="middle" className="fill-muted-foreground" fontSize={11}>
        total
      </text>
    </>
  );
      ...
      <table className="w-full text-[13px]">
        <thead>
          <tr className="border-b border-border">
            ...
            <th className="pb-2 text-right font-medium text-muted-foreground">Outreach sent</th>
            <th className="pb-2 text-right font-medium text-muted-foreground">Responded</th>
            <th className="pb-2 text-right font-medium text-muted-foreground">Response rate</th>
            <th className="pb-2 text-right font-medium text-muted-foreground">Slots filled</th>
            <th className="pb-2 text-right font-medium text-muted-foreground">Fill rate</th>
          </tr>
        </thead>
        <tbody>
          {CHANNEL_DATA.map((row) => {
            ...
            return (
              <tr key={row.channel} className="border-b border-border/50 last:border-0">
                ...
                <td className="py-3 text-right tabular-nums text-foreground">{row.sent.toLocaleString()}</td>
                <td className="py-3 text-right tabular-nums text-foreground">{row.responded.toLocaleString()}</td>
                <td className="py-3 text-right tabular-nums text-muted-foreground">{responseRate}%</td>
                <td className="py-3 text-right tabular-nums text-foreground">{row.filled.toLocaleString()}</td>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-8 pt-2">
        ...
        <section className="mb-6">
          <div className="rounded-lg border border-border bg-background">
            ...
            <div className="grid grid-cols-2 divide-x divide-y divide-border sm:grid-cols-3 xl:grid-cols-6 xl:divide-y-0">
              ...
              <SummaryMetric label="Outreach sent"       value="1,632" change="+18.4%" positive />
        ...
        {/* ── Monthly fills trend ── */}
        <section className="mb-6">
          <ChartCard title="Waitlist slots filled — monthly trend">
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={MONTHLY_DATA} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                <defs>
                  <linearGradient id="fillsGradient" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area type="monotone" dataKey="filled" stroke={C.fills} strokeWidth={2} fill="url(#fillsGradient)" name="Slots filled" />
                <Area type="monotone" dataKey="sent"   stroke="#cbd5e1" strokeWidth={1.5} fill="none" strokeDasharray="4 3" name="Outreach sent" />
              </AreaChart>
            </ResponsiveContainer>
            <Legend items={[
              { label: "Slots filled",   color: C.fills   },
              { label: "Outreach sent",  color: "#cbd5e1" },
            ]} />
          </ChartCard>
        </section>
              ...
              <ResponsiveContainer width={180} height={180}>
                <PieChart>
                  ...
                  >
                    ...
                    <text x="50%" y="50%" textAnchor="middle" dominantBaseline="central" fontSize={18} fontWeight={500} className="fill-foreground">{totalApptTypes}</text>
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
        ...
        <section className="mb-6">
          <ChartCard title="Waitlist by location">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[580px] text-[13px]">
                <thead>
                  <tr className="border-b border-border">
                    {["Location", "Slots filled", "Outreach sent", "Fill rate", "Avg fill time"].map((h) => (
                ...
                <tbody>
                  {LOCATION_ROWS.map((row) => (
                    <tr key={row.location} className="border-b border-border/50 last:border-0">
                      ...
                      <td className="py-3 text-right tabular-nums text-foreground">{row.filled}</td>
                      <td className="py-3 text-right tabular-nums text-foreground">{row.sent}</td>
                      <td className="py-3 text-right tabular-nums">
                        <span className="rounded-full bg-violet-50 px-2 py-0.5 text-[11px] font-medium text-violet-700 dark:bg-violet-950/40 dark:text-violet-400">
                          {row.fillRate}
                        </span>
                      </td>
                      <td className="py-3 text-right tabular-nums text-muted-foreground">{row.avgFill}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
